...
const (
	VK_SUCCESS VkResult = C.VK_SUCCESS
	//     VK_NOT_READY = 1,
	//     VK_TIMEOUT = 2,
	//     VK_EVENT_SET = 3,
	//     VK_EVENT_RESET = 4,
	...
	//     VK_PIPELINE_COMPILE_REQUIRED_EXT = VK_PIPELINE_COMPILE_REQUIRED,
	//     VK_ERROR_PIPELINE_COMPILE_REQUIRED_EXT = VK_PIPELINE_COMPILE_REQUIRED,
	VK_RESULT_MAX_ENUM VkResult = C.VK_RESULT_MAX_ENUM
)
...
func (o *VkExtensionProperties) copyFromCObj(p unsafe.Pointer) {
	var p1 = (*C.VkExtensionProperties)(p)
	// Type of p1.extensionName is char array ( [256]C.char )
	o.ExtensionName = C.GoString(&p1.extensionName[0])
	o.SpecVersion = uint32(p1.specVersion)
}
...
func VkEnumerateInstanceExtensionProperties(pLayerName *string, pPropertyCount *uint32, pProperties []VkExtensionProperties) VkResult {

	var pLayerName1 *C.char
	if nil != pLayerName {
		pLayerName1 = C.CString(*pLayerName)
		defer C.free(unsafe.Pointer(pLayerName1))
	}

	var propertyCount1 = C.uint32_t(*pPropertyCount)

	var properties1 []C.VkExtensionProperties
	var pProperties1 *C.VkExtensionProperties
	if nil != pProperties && *pPropertyCount > 0 {
		properties1 = make([]C.VkExtensionProperties, *pPropertyCount)
		pProperties1 = &properties1[0]
	}

	var result1 = C.vkEnumerateInstanceExtensionProperties(
		pLayerName1,
		&propertyCount1,
		pProperties1,
	)

	var result = VkResult(result1)
	if VK_SUCCESS != result {
		return result
	}

	*pPropertyCount = uint32(propertyCount1)

	if nil != pProperties && *pPropertyCount > 0 {
		for i := 0; i < int(*pPropertyCount); i++ {
			pProperties[i].copyFromCObj(unsafe.Pointer(&properties1[i]))
		} // for
	}
