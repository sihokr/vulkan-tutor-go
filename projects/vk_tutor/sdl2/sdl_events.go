...
type SDL_Event struct {
	cObjData [C.sizeof_SDL_Event]byte
}

func (o *SDL_Event) Type() SDL_EventType {
	var p = (*C.Uint32)(unsafe.Pointer(&o.cObjData[0]))
	return SDL_EventType(*p)
}
...
func SDL_PollEvent(event *SDL_Event) int {
	var p = (*C.SDL_Event)(unsafe.Pointer(&event.cObjData[0]))
	return int(C.SDL_PollEvent(p))
}
